class BrowseController < ApplicationController
  ...
  def browse
    like_account_ids = Like.where(account_id: current_account.id).map(&:like_account_id)
  ...
  def approve
    ...
    new_like = like.new(like_account_id: account_id)
    new_like.account_id = current_account.id

    if new_like.save
      # check if user already likes us back
      existing_like = like.where(account_id: account_id,like_account_id: current_account.id).count
